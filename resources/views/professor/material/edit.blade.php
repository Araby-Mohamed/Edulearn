    ...
    <!-- Start Content -->
        ...
        <form method="post" action="{{url('material/update/'.$material->id)}}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input type="text" class="form-control" id="title" name="lecture" value="{{$material->lecture}}" placeholder="Enter Lecture">
            </div>
            <div class="form-group">
                <label for="content">Content</label>
                <textarea class="form-control" name="content" id="content" placeholder="Enter Your Details">{{$material->content}}</textarea>
            </div>
            <div class="form-group">
                <label for="content">Subject</label>
                <select name="subject_id" class="form-control">
                    @foreach($subject as $sub)
                        <option value="{{($sub->id)}}" {{($sub->id == $material->subject_id ? 'selected' : '')}}>{{$sub->name . ' ('.$sub->code.')'}}</option>
                    @endforeach
                </select>
            </div>
            <div class="form-group">
