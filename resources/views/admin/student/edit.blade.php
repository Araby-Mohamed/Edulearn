...
<div class="portlet light bordered">
    <div class="portlet-title">
        <div class="caption">
            ...
            <span class="caption-subject font-green-sharp bold uppercase">Edit Student</span>
        </div>
    </div>
    @include('admin.layouts.message')
    <div class="portlet-body form">
        {{--<form action="{{url('admin/Student/update',$student->id)}}" method="post" role="form">--}}
        <form action="{{url('admin/student',$user->id)}}" method="post" enctype="multipart/form-data" role="form">
            @csrf
            @method('PUT')
            <div class="form-body">
                    ...
                    <div class="mt-checkbox-inline">
                        @foreach($score as $sc)
                            <label class="mt-checkbox">
                                <input type="checkbox" id="inlineCheckbox1" checked disabled> {{$sc->subject->name}}
                                <span></span>
                            </label>
                        @endforeach
                    </div>
                ...
                @if(count($diff) > 0 )
                <div class="form-group">
                    <label>Select New Subject</label>
                    <div class="mt-checkbox-inline">
                        @foreach($diff as $dif)
                            <label class="mt-checkbox">
                                <input type="checkbox" id="inlineCheckbox1" value="{{$dif->id}}" name="subject_id[]"> {{$dif->name}}
                                <span></span>
                            </label>
                        @endforeach
                    </div>
                    ...
                    <div class="col-md-12">
                        <div class="fileinput fileinput-new" data-provides="fileinput">
                            <div class="fileinput-new thumbnail" style="width: 200px; height: 150px;">
                                @if($user->image != null)
                                    <img src="{{url('media/images/student/'.$user->image)}}" alt="" />
                                @else
